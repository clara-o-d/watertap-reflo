...
def create_heat_map(csv_file_path, flow_volume, evaporation_rate):
    ...
    cbar.ax.set_ylabel('Levelized cost (thousands $/t Li⁺)', fontsize=18)
    ...
    plt.xlabel('Evaporation rate (mm/year)', fontsize=18)
    plt.ylabel('Inlet Li$^+$ concentration (%)', fontsize=18)
    plt.title(f'Li$^+$ brine levelized cost estimate\nand % change', fontsize=18, fontweight='bold')
