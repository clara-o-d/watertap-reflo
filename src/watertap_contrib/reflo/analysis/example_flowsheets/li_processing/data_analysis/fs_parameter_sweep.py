...
def build_sweep_params(m, **kwargs):
    ...
    #     m.fs.magnesium_removal_fraction_soda_ash_reactor, 0.50, 0.90, 3
    # )
    ...
    #     m.fs.lithium_removal_fraction_lithium_reactor, 0.3, 0.7, 3
    # )
    ...
    #     m.fs.soda_ash_reactor.reaction_rate_constant_mg, 0.0001, 0.001, 3
    # )

    # sweep_params['Lime reactor reaction constant'] = LinearSample(
    #     m.fs.lime_reactor.reaction_rate_constant_mg, 0.001, 0.002, 3
    # )

    # sweep_params['Lithium reactor reaction constant'] = LinearSample(
    #     m.fs.lithium_carbonate_reactor.reaction_rate_constant_li, 0.0001, 0.001, 3
    # )

    # sweep_params['Storage tank surge capacity'] = LinearSample(
    #     m.fs.brine_storage.surge_capacity[0], 0, 0.5, 3
    # )

    sweep_params['Soda Ash Dewatering Split Fraction H2O'] = LinearSample(
        m.fs.soda_ash_vacuum_filter.split_fraction[0, "overflow", "H2O"], 0.8, 0.99, 3
    )

    sweep_params['Feed density (kg/m3)'] = LinearSample(
        m.fs.brine_feed.properties[0].dens_mass_phase["Liq"], 1200, 1400, 3
    )
...
def build_outputs(m, **kwargs):
    ...
    outputs['Storage tank surge capacity'] = m.fs.brine_storage.surge_capacity[0]
    outputs['Soda Ash Dewatering Split Fraction H2O'] = m.fs.soda_ash_vacuum_filter.split_fraction[0, "overflow", "H2O"]
    outputs['Feed density (kg/m3)'] = m.fs.brine_feed.properties[0].dens_mass_phase["Liq"]
...
if __name__ == "__main__":
    ...
    parameter_sweep(
        ...
        build_sweep_params, 
        build_outputs,
        csv_results_file_name='parameter_sweep022526_4.csv', 
